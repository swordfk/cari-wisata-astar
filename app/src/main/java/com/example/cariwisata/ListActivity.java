package com.example.cariwisata;
...
import com.example.cariwisata.app.AppController;
        ...
        LocationListener
{
    ...
    private void getMarkers() {
        String url = "https://apiastar.000webhostapp.com/index.php?from="+lastLat+","+lastLng;
                ...
                try {
                    ...
                    if(tmpRadius > 2147483647){
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            keterangan = jsonObject.getString(KET);
                            end_lat = Double.parseDouble(jsonObject.getString(LAT));
                            end_lng = Double.parseDouble(jsonObject.getString(LNG));

                            // Menambah data marker untuk di tampilkan ke google map
                            addMarker(end_lat, end_lng, title, keterangan);
                        }

                        if(getAreaDone > 0){
                            if(getAreaDone == maxArea){
                                Toast.makeText(ListActivity.this, "Ditemukan "+String.valueOf(getAreaDone)+" Wisata terdekat (MAX "+maxArea+" lokasi)",
                                        Toast.LENGTH_LONG).show();
                            }else{
                                Toast.makeText(ListActivity.this, "Ditemukan "+String.valueOf(getAreaDone)+" Wisata terdekat", Toast.LENGTH_LONG).show();
                            }
                            ...
                            Toast.makeText(ListActivity.this, "Wisata tidak ditemukan dalam radius ("+String.valueOf(lengthRadius)+" m), silahkan ubah nilai Radius",
                                    Toast.LENGTH_LONG).show();
                        }
                    }
    ...
    private void addMarker(double mark_lat, double mark_lng, final String title, final String keterangan) {
        ...
        if(results[0] <= lengthRadius){
            ...
            vPlaces.add(title+" ("+String.format("%.2f", results[0])+"m)");
            ...
            mMap.setOnInfoWindowClickListener(new GoogleMap.OnInfoWindowClickListener() {
                ...
                public void onInfoWindowClick(Marker marker) {
                    Toast.makeText(ListActivity.this, keterangan, Toast.LENGTH_SHORT).show();
                }
            });
        }
